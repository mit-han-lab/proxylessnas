...
def conv2d(_input, out_features, kernel_size, stride=1, padding='SAME', param_initializer=None):
    ...
    with tf.variable_scope('conv'):
        ...
        weight = tf.get_variable(
            name='weight', shape=[kernel_size, kernel_size, in_features, out_features],
        ...
        with tf.variable_scope(self.id):
            if self.expand_ratio > 1:
                ...
                with tf.variable_scope('inverted_bottleneck'):
                    ...
                    output = batch_norm(output, net.is_training,
                                        epsilon=net.bn_eps, decay=net.bn_decay, param_initializer=init)
                    output = activation(output, 'relu6')
            with tf.variable_scope('depth_conv'):
                output = depthwise_conv2d(output, self.kernel_size, self.stride, param_initializer=init)
                output = batch_norm(output, net.is_training,
                                    epsilon=net.bn_eps, decay=net.bn_decay, param_initializer=init)
                output = activation(output, 'relu6')
...
class ConvLayer:
    ...
    def build(self, _input, net, init=None):
        output = _input
        with tf.variable_scope(self.id):
            output = conv2d(output, self.filter_num, self.kernel_size, self.stride, param_initializer=init)
            output = batch_norm(output, net.is_training,
                                epsilon=net.bn_eps, decay=net.bn_decay, param_initializer=init)
            output = activation(output, 'relu6')
        return output
...
class LinearLayer:
    ...
    def build(self, _input, net, init=None):
        ...
        with tf.variable_scope(self.id):
            ...
            output = fc_layer(output, self.n_units, use_bias=True, param_initializer=init)
        return output
