...
def _make_backbone():
    ...
    import torch
    json_file = osp.join(osp.split(osp.abspath(__file__))[0], "proxyless-w0.25-r112_imagenet.json")
    with open(json_file) as f:
        config = json.load(f)
    _model = ProxylessNASNets.build_from_config(config)
    ckpt = torch.load(osp.join(osp.split(osp.abspath(__file__))[0], "proxyless-w0.25-r112_imagenet.pth"))
